...
ggplot(realestate_train, aes(x=house_age, y=price)) + geom_point(size=2, color = "#00BFC4", shape=19) + geom_smooth(method='lm', formula= y~x, se = FALSE, colour="cyan")

ggplot(realestate_train, aes(x=house_age, y=price)) + geom_point(size=2, color = "#00BFC4", shape=19) + geom_smooth(method='lm', formula= y~poly(x,2), se = FALSE, colour="cyan")
...
ggplot(realestate_test, aes(x=house_age, y=price)) + geom_point(size=2, color = "#F8766D" , shape=19) + geom_line(aes(x = house_age, y = pred_val_test), color = "red", size = 1.5)
...
K <- seq(3, N)
rmse_val <- seq(3, N)
price_train <- as.vector(realestate_train$price)
for (i in 1:(N-2))
{
  knnfit <- knn.reg(train = realestate_train$house_age, y = realestate_train$price, k = K[i])
